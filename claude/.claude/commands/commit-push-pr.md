...
5. Commit, then push: `git commit`, then `git push -u origin HEAD`. Commits are **not
   signed** and `sandbox.excludedCommands` is empty, so `git commit` runs fully inside
   the sandbox — pre-commit hooks included. `git -C <dir> commit` is fine too.
